...
const Home = () => {
  ...
  const [showFoodItemsInSearch, setShowFoodItemsInSearch] =
    ...
    if (restaurants && restaurants.length > 0) {
      const fetchFoodItems = async () => {
        ...
        for (const restaurant of restaurants) {
          try {
            const response = await fetch(
              `/api/restaurants/${restaurant.id}/food-items`,
            );
            if (response.ok) {
            ...
            console.error(
              `Error fetching food items for restaurant ${restaurant.id}:`,
              error,
            );
          }
        }
        ...
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="text-right">
              ...
              <div className="flex items-center">
                <span className="font-medium text-[#4F2D1F] text-sm">
                  {userLocation.name}
                </span>
                <button
                  onClick={() => {
                    const detectLocationFn = () => {
                      navigator.geolocation.getCurrentPosition(
                        ...
                        (error) => {
                          console.error("Error getting location:", error);
                          alert(
                            "Please allow location access to improve delivery accuracy",
                          );
                        },
                      );
                    };
                    ...
                    restaurants.filter(
                      ...
                      <>
                          ...
                          restaurants
                            ...
                            .map((restaurant, index) => (
                              ...
                              >
                                ...
                                <div>
                                  <div className="font-medium text-sm text-[#4F2D1F]">
                                    {restaurant.name}
                                  </div>
                        ...
                        {foodItems.filter((item) =>
                          ...
                          <div className="border-t border-[#E5A764]/20 my-2"></div>
                        )}
                      </>
                    )}
      ...
      >
          ...
          <div className="bg-gradient-to-r from-[#8B572A] to-[#4F2D1F] p-5 flex">
            <div className="flex-grow">
              <h3 className="text-xl font-bold text-white font-dm-sans">
                Try Authentic Ethiopian Doro Wat
              </h3>
              <p className="text-sm text-white/90 mb-2">
                A spicy, rich Ethiopian chicken stew with berbere spices and
                boiled egg
              </p>
            ...
            <div className="w-1/3 relative overflow-hidden rounded-lg">
              ...
              <img
                src="https://ministryofcurry.com/wp-content/uploads/2020/02/doro-wat-4.jpg"
        ...
        >
          ...
          <div className="mt-3 flex items-center justify-between">
            <div>
              <p className="text-sm text-[#4F2D1F] font-medium">
                Ethiopian restaurants near you
              </p>
              <p className="text-xs text-[#8B572A]">
                Showing restaurants within 3km
              </p>
            </div>
